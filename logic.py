...
class Model():
    ...
    def extend(self, **pairs):
        return Model({**self.model, **pairs})
    
    def symbols(self):
        return set(self.model.keys())
    
    def copy(self):
        return Model(self.model.copy())
    
    def __eq__(self, other):
        return isinstance(other, Model) and self.model == other.model
    
    def __hash__(self):
        return hash(tuple(sorted(self.model.items())))
    
def all_models(variables):
    """Returns an iterator over all possible models for a given set of variables."""
    for values in itertools.product([True, False], repeat=len(variables)):
        yield Model(zip(variables, values))

def entails(knowledge, query):
    """Checks if knowledge base entails the query."""
    for model in all_models(knowledge.symbols() | query.symbols()):
...
class Sentence():
    ...
    def validate(cls, sentence):
        if not isinstance(sentence, Sentence):
            raise TypeError("must be logical sentence.")
        
    @classmethod
    def parenthesize(cls, s):
        """Parenthesizes the expression if not already parenthesized."""
        def balanced(s):
...
def model_check(knowledge, query):
    ...
    def check_all(knowledge, query, symbols, model):
        """Checks if knowledge base entails the query, given a model."""
        if not symbols:
            if knowledge.evaluate(model):
                return query.evaluate(model)
            return True
        else:
            # Choose one of the remaining symbols
